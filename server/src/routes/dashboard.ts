...
export function dashboardRouter(db: DB): Router {
  ...
  r.get('/', (req, res) => {
    ...
    const cs = s.cycle_tracking ? buildCycleSummary(db, today) : null;
    res.json({
      settings: s,
      ...
      cycle: cs
        ? {
            prompt: cyclePrompt(cs, today),
            predicted_start: cs.predicted_start,
            concern_date: cs.concern_date,
            is_late: cs.is_late,
            open_entry: cs.open_entry,
            avg_duration_days: cs.avg_duration_days,
          }
        : null,
    });
  });
