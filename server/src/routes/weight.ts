...
import { finiteNum, isDayStr, nowIso, todayStr } from '../util';
...
export function weightRouter(db: DB): Router {
  ...
  r.post('/', (req, res) => {
    ...
    const date = isDayStr(b.entry_date) ? b.entry_date : todayStr();
    const weight = finiteNum(b.weight_lb);
    if (weight == null || weight < 20 || weight > 1500) return res.status(400).json({ error: 'weight_lb must be a sane number' });
    const ts = nowIso();
    db.prepare(
  ...
  // Fix a typo without deleting and re-entering.
  r.patch('/:id', (req, res) => {
    const id = Number(req.params.id);
    const existing = db.prepare('SELECT * FROM weight_entries WHERE id = ?').get(id) as Record<string, any> | undefined;
    if (!existing) return res.status(404).json({ error: 'not_found' });
    const b = (req.body ?? {}) as Record<string, any>;
    const weight = b.weight_lb != null ? finiteNum(b.weight_lb) : existing.weight_lb;
    if (weight == null || weight < 20 || weight > 1500) return res.status(400).json({ error: 'weight_lb must be a sane number' });
    db.prepare('UPDATE weight_entries SET weight_lb = ?, note = ?, updated_at = ? WHERE id = ?').run(weight, 'note' in b ? (b.note ?? null) : existing.note, nowIso(), id);
    recomputeTrend(db);
    writeAudit(db, { entity: 'weight', entityId: id, action: 'update' });
    res.json(db.prepare('SELECT * FROM weight_entries WHERE id = ?').get(id));
  });
  ...
  r.put('/goal', (req, res) => {
    ...
    if ('start_lb' in b) partial.weight_start_lb = b.start_lb == null ? null : finiteNum(b.start_lb);
    if ('target_lb' in b) partial.weight_target_lb = b.target_lb == null ? null : finiteNum(b.target_lb);
